...
shopifyRouter.config = {
    ...
    redirect_uri:shopifyRouter.redirectUri
                            ...
                            ///How to insert new doc using mongoose
...
shopifyRouter.get('/activateRecurringCharge',function(req, res, next){
    ...
    dbShopRecurringChargeDetail.findOne({"recurring_application_charge.id": parseInt(chargeId) }, function(err,obj) {
        if(err){
            console.log("err while activation :"+err);
            res.send("err while activation :"+err);
        }else{
                    ...
                    if(err){ res.send("error while geting token"); }else{
                        if(status == "found"){
                            ...
                            Shopify.post('/admin/recurring_application_charges/'+chargeId+'/activate.json',recurringChargeDetail,function(err,result,header){
                                ...
                                ///update some fields(etc status) in db  than redirect
                            ...
                            console.log("token not found while Activation,First Install App in this Store");
                            res.send("token not found while Activation,First Install App in this Store");
                        }
                    }
                    
                });
            }else{ res.send("No charge in db to activate ,Means first create a Recuriing Charge for this Store");}
        }
...
shopifyRouter.get('/finish_auth',function (req,res,next) {
    ...
    getShopToken(shopifyRouter.shop,function(err,status,token){
        if (err) {
            ...
            if(status == "not found"){
                delete shopifyRouter.config['access_token'];  ///Delete If any previous access_token 
